            ...
            if cached and cached.get('total_files', 0) > 0 and cached.get('patterns') and cached.get('arch_macro'):
                ...
                return {
                    ...
                    'architecture': {
                        ...
                        'micro': cached.get('architecture', {}).get('micro', ''),
                    },
                    'status': 'completed',
                    'cached': True
                }
                ...
                if self._is_snapshot_complete(existing['snapshot_id']):
                    ...
                    return {
                        ...
                        'architecture': {
                            ...
                            'micro': existing.get('architecture', {}).get('micro', ''),
                        },
                        'status': 'cached'
                    }
                else:
...
3. Key risks to consider"""
    ...
    def _has_uncommitted_changes(self, repo_path: str) -> bool:
        ...
        try:
            ...
            logger.warning("⚠️ Git is not installed — cannot check for uncommitted changes")
            return False
        except Exception:
            return False
